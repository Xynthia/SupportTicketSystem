...
namespace SupportTicketSystem.Services.JoinUserTicketService.ExtensionMethods
{
    ...
    {
            public static async Task<List<GetJoinUserTicketDto>> GetJoinTicketDtoFromQuery(this IQueryable<JoinUserTicket> query, IMapper mapper)
            {
                return await query.Select(c => mapper.Map<GetJoinUserTicketDto>(c)).ToListAsync();
            }
